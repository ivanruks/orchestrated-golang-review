...
- `{{tmp_dir}}` — temp dir (e.g., `/tmp/go-review/2026-04-03T14-30_branch-feature-xyz/`)
- `{{output_dir}}` — persistent dir (e.g., `docs/review/2026-04-03T14-30_branch-feature-xyz/`)
...
Generate diffs from local git, save to `{{tmp_dir}}`.
...
Save as `commit_log` for metadata.
...
Split into per-file diffs. Exclude EXCLUDED_FILE_PATTERNS. Save each to `{{tmp_dir}}/diffs/<path-with-dashes>.diff`. Track: path, lines added/removed.

No `.go` files → stop: "No Go files changed between `{{source_branch}}` and `{{target_branch}}`."
...
- No `.go` in diff → "No Go files changed between {{source_branch}} and {{target_branch}}", stop
